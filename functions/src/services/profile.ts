...
export async function updateStudentProfile(
  ...
  const oldStrengths: string[] = currentProfile?.strengths || [];
  const oldWeaknesses: string[] = currentProfile?.weaknesses || [];
  const oldSummary: string = currentProfile?.learningSummary || "";
...
Kết quả JSON:`;
  ...
  try {
    ...
    console.log(
      `[Memory] Updated student profile for uid: ${uid}. Added strengths: ${JSON.stringify(
        newStrengths
