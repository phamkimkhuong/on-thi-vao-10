...
import { g10BiologyVideos, BiologyChapterVideo } from '../../../data/grade10/biology/videos';
import { Card, CardContent, CardHeader, CardTitle } from '../../../components/ui/card';
import { Play, Tv, Sparkles } from 'lucide-react';
import { cn } from '../../../utils/cn';

export const BiologyVideoDashboard: React.FC = () => {
  const [activeVideo, setActiveVideo] = useState<BiologyChapterVideo>(g10BiologyVideos[0]);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-linear-to-r from-green-500/5 via-emerald-500/5 to-teal-500/5 border border-green-500/10 p-6 flex items-center justify-between gap-6">
        <div className="space-y-1.5 z-10">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-green-500/10 border border-green-500/20 text-xs font-black text-green-600 dark:text-green-400 uppercase tracking-wider mb-1">
          ...
          <p className="text-xs md:text-sm text-muted-foreground font-semibold">
            Tận dụng sức mạnh của hình ảnh và âm thanh giúp bạn hiểu sâu hơn cấu trúc tế bào, quá trình phân bào và các hoạt động sống lý thú.
          </p>
        </div>
        <div className="hidden sm:block p-4 bg-green-500/10 rounded-full border border-green-500/20 text-green-500 shrink-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 xl:col-span-8 space-y-6">
          ...
          <Card className="border-green-500/10 shadow-lg rounded-2xl bg-card relative overflow-hidden">
            <div className="absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-green-500 to-emerald-500" />
            <CardHeader className="p-6 pb-3">
              <span className="text-[10px] font-black text-green-600 dark:text-green-400 tracking-widest uppercase block mb-1">
            ...
            <CardContent className="p-6 pt-0 space-y-4 text-xs font-semibold leading-relaxed text-muted-foreground">
              ...
              <div className="p-4 bg-green-500/[0.02] dark:bg-green-950/[0.02] border border-green-500/10 rounded-2xl flex items-start gap-3">
                <span className="text-green-500 shrink-0 text-sm mt-0.5">💡</span>
                <p className="text-[11px] font-semibold text-green-700/90 dark:text-green-400/90">
        ...
        <div className="lg:col-span-5 xl:col-span-4 space-y-4">
          <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest block pl-1">
            DANH SÁCH BÀI GIẢNG ({g10BiologyVideos.length} CHƯƠNG):
          </label>
          
          <div className="space-y-3 max-h-[500px] overflow-y-auto pr-1.5 scrollbar-thin">
            {g10BiologyVideos.map((video, index) => {
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isPlaying
                      ? "border-green-500 shadow-md shadow-green-500/5 bg-gradient-to-br from-card to-green-500/[0.01] ring-1 ring-green-500/25"
                      : "border-border/65 hover:border-green-500/40 hover:-translate-y-0.5 hover:shadow-xs"
                  )}
                >
                  <div className="flex items-center gap-3.5 min-w-0">
                    <div className={cn(
                      ...
                      isPlaying
                        ...
                        : "bg-secondary text-muted-foreground group-hover:bg-green-500/10 group-hover:text-green-500"
                    )}>
                      {isPlaying ? (
                  ...
                  <span className={cn(
                    ...
                    isPlaying
                      ? "bg-green-500/10 border-green-500/25 text-green-600 dark:text-green-400 animate-pulse"
...
export default BiologyVideoDashboard;
