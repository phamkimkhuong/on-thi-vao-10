...
import { g10PhysicsVideos, PhysicsChapterVideo } from '../../../data/grade10/physics/videos';
import { Card, CardContent, CardHeader, CardTitle } from '../../../components/ui/card';
import { Play, Tv, Sparkles } from 'lucide-react';
import { cn } from '../../../utils/cn';

export const PhysicsVideoDashboard: React.FC = () => {
  const [activeVideo, setActiveVideo] = useState<PhysicsChapterVideo>(g10PhysicsVideos[0]);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-linear-to-r from-blue-500/5 via-sky-500/5 to-indigo-500/5 border border-blue-500/10 p-6 flex items-center justify-between gap-6">
        <div className="space-y-1.5 z-10">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-xs font-black text-blue-600 dark:text-blue-400 uppercase tracking-wider mb-1">
          ...
          <p className="text-xs md:text-sm text-muted-foreground font-semibold">
            Tận dụng sức mạnh của hình ảnh và âm thanh giúp bạn hiểu sâu hơn các hiện tượng cơ học, lực học và các định luật bảo toàn.
          </p>
        </div>
        <div className="hidden sm:block p-4 bg-blue-500/10 rounded-full border border-blue-500/20 text-blue-500 shrink-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 xl:col-span-8 space-y-6">
          ...
          <Card className="border-blue-500/10 shadow-lg rounded-2xl bg-card relative overflow-hidden">
            <div className="absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-blue-500 to-indigo-500" />
            <CardHeader className="p-6 pb-3">
              <span className="text-[10px] font-black text-blue-600 dark:text-blue-400 tracking-widest uppercase block mb-1">
            ...
            <CardContent className="p-6 pt-0 space-y-4 text-xs font-semibold leading-relaxed text-muted-foreground">
              ...
              <div className="p-4 bg-blue-500/[0.02] dark:bg-blue-950/[0.02] border border-blue-500/10 rounded-2xl flex items-start gap-3">
                <span className="text-blue-500 shrink-0 text-sm mt-0.5">💡</span>
                <p className="text-[11px] font-semibold text-blue-700/90 dark:text-blue-400/90">
        ...
        <div className="lg:col-span-5 xl:col-span-4 space-y-4">
          <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest block pl-1">
            DANH SÁCH BÀI GIẢNG ({g10PhysicsVideos.length} BÀI):
          </label>
          
          <div className="space-y-3 max-h-[500px] overflow-y-auto pr-1.5 scrollbar-thin">
            {g10PhysicsVideos.map((video, index) => {
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    isPlaying
                      ? "border-blue-500 shadow-md shadow-blue-500/5 bg-gradient-to-br from-card to-blue-500/[0.01] ring-1 ring-blue-500/25"
                      : "border-border/65 hover:border-blue-500/40 hover:-translate-y-0.5 hover:shadow-xs"
                  )}
                >
                  <div className="flex items-center gap-3.5 min-w-0">
                    <div className={cn(
                      ...
                      isPlaying
                        ...
                        : "bg-secondary text-muted-foreground group-hover:bg-blue-500/10 group-hover:text-green-500"
                    )}>
                      {isPlaying ? (
                  ...
                  <span className={cn(
                    ...
                    isPlaying
                      ? "bg-blue-500/10 border-blue-500/25 text-blue-600 dark:text-blue-400 animate-pulse"
...
export default PhysicsVideoDashboard;
